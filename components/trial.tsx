...
function createBeam(width: number, height: number, isDarkMode: boolean): Beam {
  const angle = -35 + Math.random() * 10;
  // Red hues: 0–20 for true red to red-orange; dark mode leans cooler crimson
  const hueBase = isDarkMode ? 340 : 0;
  const hueRange = isDarkMode ? 30 : 20;
...
export default function BeamsBackground({
  ...
  useEffect(() => {
    ...
    function resetBeam(beam: Beam, index: number, totalBeams: number) {
      ...
      // Red hue range: dark mode = deep crimson/rose (340–10), light = vivid red (0–20)
      const hueBase = isDarkModeRef.current ? 340 : 0;
      const hueRange = isDarkModeRef.current ? 30 : 20;
    ...
    function drawBeam(ctx: CanvasRenderingContext2D, beam: Beam) {
      ...
      // Dark mode: high saturation, mid-high lightness for vivid crimson glow
      // Light mode: very high saturation, lower lightness for deep concentrated red
      const saturation = isDarkModeRef.current ? "90%" : "60%";
      const lightness = isDarkModeRef.current ? "60%" : "55%";
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        className="absolute inset-0 bg-red-900/5 dark:bg-red-950/5"
        style={{
          backdropFilter: "blur(50px)",
        }}
        transition={{
